...
class Agent {
    ...
    resolveReplanningCycleLimit(modelConfigForPlanning) {
        ...
        return (0, util_js_namespaceObject.isUITars)(modelConfigForPlanning.modelFamily) ? defaultVlmUiTarsReplanningCycleLimit : (0, util_js_namespaceObject.isAutoGLM)(modelConfigForPlanning.modelFamily) ? defaultAutoGlmReplanningCycleLimit : defaultReplanningCycleLimit;
    }
    ...
    async aiTap(locatePrompt, opt) {
        ...
        return (0, external_tasks_js_namespaceObject.withFileChooser)(this.interface, fileChooserAccept, async ()=>this.callActionInActionSpace('Tap', {
    ...
    async aiAct(taskPrompt, opt) {
        ...
        const runAiAct = async ()=>{
            ...
            const isVlmUiTars = (0, util_js_namespaceObject.isUITars)(modelConfigForPlanning.modelFamily);
            const isAutoGlm = (0, util_js_namespaceObject.isAutoGLM)(modelConfigForPlanning.modelFamily);
            ...
            const { output: actionOutput } = await this.taskExecutor.action(taskPrompt, modelConfigForPlanning, defaultIntentModelConfig, includeBboxInPlanning, this.aiActContext, cacheable, replanningCycleLimit, imagesIncludeCount, deepThink, fileChooserAccept, deepLocate, abortSignal);
            if (this.taskCache && actionOutput?.yamlFlow?.length && false !== cacheable) {
                const yamlContent = {
                    tasks: [
                        {
                            name: taskPrompt,
    ...
    async recordToReport(title, opt) {
        ...
        const now = Date.now();
        const screenshot = external_screenshot_item_js_namespaceObject.ScreenshotItem.create(base64, now);
        const recorder = [
            {
                type: 'screenshot',
                ts: now,
    ...
    normalizeFilePaths(files) {
        if (utils_namespaceObject.ifInBrowser) throw new Error('File chooser is not supported in browser environment');
        return files.map((file)=>{
            const absolutePath = (0, external_node_path_namespaceObject.resolve)(file);
            if (!(0, external_node_fs_namespaceObject.existsSync)(absolutePath)) throw new Error(`File not found: ${file}. Resolved to: ${absolutePath}. Current working directory: ${process.cwd()}`);
            return absolutePath;
        });
    }
